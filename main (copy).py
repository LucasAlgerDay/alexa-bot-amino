...
def summ(data):
	try:
		resp=requests.get('https://replit.com/@samosa8811/LightcoralGrayNagware-2/sig-gen/')
...
def ping(data):
	try:
		o=measure_latency(host='alexa.jsjsisidi.repl.co')
		for x in o:
			i=int(x)
...
def latency(data):
	try:
		x=measure_latency(host='alexa.jsjsisidi.repl.co')
		for u in x:
			i=int(u)
...
def img(args):
	try:
		...
		Image.open(f"Dow/{s}").resize((600,700)).save("imgi.png")
		ff=open("imgi.png","rb")
...
def pic(args):
  ...
  #	imfile= f"my_img{randint(1,500)}"
  	search =args.message
  	gis = GoogleImagesSearch('AIzaSyB4Ekh8Z3Ie99Qt9V7Rui3dxbTJ0N2qXCc', '42e3ecc0e1ca6412f')
  	_search_params = {'q': search,'num': 6,'safe':'high'}
  	try:
